...
def handler(event, context):
    ...
    try:
        ...
        post_data = dynamodb_resource.Table(TABLE_NAME).get_item(Key={'UserId': user_id, 'Timestamp': int(timestamp)}).get('Item', {})
        
        def post_tweet(token):
            return requests.post("https://api.x.com/2/tweets", headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}, json={"text": post_data.get('AgentResponse')})

        res = post_tweet(brand_data.get('x_access_token'))
        if res.status_code == 401:
            new_at = refresh_x_token(user_id, brand_data.get('x_refresh_token'))
            if new_at: res = post_tweet(new_at)

        if res.status_code == 201:
            dynamodb_resource.Table(TABLE_NAME).update_item(Key={'UserId': user_id, 'Timestamp': int(timestamp)}, UpdateExpression="SET #s = :s, TweetId = :tid", ExpressionAttributeNames={'#s': 'Status'}, ExpressionAttributeValues={':s': 'PUBLISHED', ':tid': res.json()['data']['id']})
