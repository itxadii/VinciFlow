...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleSignOut = () => {
    if (onSignOut) onSignOut(); // 1. Pehle session clear karo
    navigate('/');
  };
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* 1. Header: pt-20 Margin + Handlee Logo Link */}
        <div className="pt-10 pb-4 px-4 border-b border-slate-400/40 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            {/* Logo: Clickable to take back to Landing Page */}
        ...
        {/* 2. Chat History: Merriweather Bold */}
            ...
            <button
              ...
              className={`w-full text-left p-3 rounded-xl transition-all font-['Merriweather'] font-bold ${
                currentSessionId === session.sessionId 
                  ? 'bg-slate-400/50 text-slate-900 shadow-sm border border-slate-400/50' 
        ...
        <div className="p-4 border-t border-slate-400/40 bg-slate-400/10 font-['Merriweather'] font-bold">
          <div className="flex items-center gap-3 px-1 mb-4">
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              <span className="text-[10px] text-slate-600 uppercase">Pro Developer</span>
            </div>
          </div>
          <button 
            onClick={handleSignOut}
            className="w-full py-2.5 px-3 flex items-center justify-center gap-2 text-xs font-bold text-slate-700 hover:text-red-600 hover:bg-red-50/50 rounded-xl transition-all border border-transparent hover:border-red-100"
          >
